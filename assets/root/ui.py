...
class ImageBox(Window):
    ...
    def SetEvent(self, func, *args):
        if not args:
            print("[ERROR] ui.py SetEvent, missing event key")
            return
        key = args[0]
        if key in self.eventFunc:
            self.eventFunc[key] = func
            self.eventArgs[key] = args
        else:
            print("[ERROR] ui.py SetEvent, Can`t Find event key : %s" % key)

    def SAFE_SetEvent(self, func, *args):
        if not args:
            print("[ERROR] ui.py SAFE_SetEvent, missing event key")
            return
        key = args[0]
        if key in self.eventFunc:
            self.eventFunc[key] = __mem_func__(func)
            self.eventArgs[key] = args
        else:
            print("[ERROR] ui.py SAFE_SetEvent, Can`t Find event key : %s" % key)

    def OnMouseLeftButtonUp(self):
        if self.eventFunc["MOUSE_LEFT_BUTTON_UP"]:
            self.eventFunc["MOUSE_LEFT_BUTTON_UP"](*self.eventArgs["MOUSE_LEFT_BUTTON_UP"])

    def OnMouseLeftButtonDown(self):
        if self.eventFunc["MOUSE_LEFT_BUTTON_DOWN"]:
            self.eventFunc["MOUSE_LEFT_BUTTON_DOWN"](*self.eventArgs["MOUSE_LEFT_BUTTON_DOWN"])

    def OnMouseRightButtonUp(self):
        if self.eventFunc["MOUSE_RIGHT_BUTTON_UP"]:
            self.eventFunc["MOUSE_RIGHT_BUTTON_UP"](*self.eventArgs["MOUSE_RIGHT_BUTTON_UP"])

    def OnMouseRightButtonDown(self):
        if self.eventFunc["MOUSE_RIGHT_BUTTON_DOWN"]:
            self.eventFunc["MOUSE_RIGHT_BUTTON_DOWN"](*self.eventArgs["MOUSE_RIGHT_BUTTON_DOWN"])
...
class Button(Window):
    ...
    if getattr(app, "ENABLE_ADVANCED_GAME_OPTIONS", False):
        def SetRenderingRect(self, left, top, right, bottom):
            wndMgr.SetRenderingRect(self.hWnd, left, top, right, bottom)
    ...
    def SetTextAlignLeft(self, text, x=8, y=0):
        if not self.ButtonText:
            textLine = TextLine()
            textLine.SetParent(self)
            textLine.SetPosition(x, self.GetHeight() // 2 + y)
            textLine.SetVerticalAlignCenter()
            ...
            self.ButtonText.SetPosition(x, self.GetHeight() // 2 + y)
    ...
    def OnMouseOverIn(self):
        if self.overFunc:
            self.overFunc(*self.overArgs)

    def OnMouseOverOut(self):
        if self.overOutFunc:
            self.overOutFunc(*self.overOutArgs)
    ...
    def ShowToolTip(self):
        ...
        if self.showtooltipevent:
            self.showtooltipevent(*self.showtooltiparg)

    def HideToolTip(self):
        if self.ToolTipText:
            self.ToolTipText.Hide()
        if self.hidetooltipevent:
            self.hidetooltipevent(*self.hidetooltiparg)
...
class ThinBoard(Window):
    ...
    LINE_WIDTH = 16
    LINE_HEIGHT = 16
    BOARD_COLOR = grp.GenerateColor(0.0, 0.0, 0.0, 0.65)
...
class ListBox(Window):
    ...
    def OnRender(self):
        ...
        if -1 != self.overLine:
            ...
            grp.RenderBar(int(xRender + 2), int(yRender + self.overLine*self.stepSize + 4), int(self.width - 3), int(self.stepSize))
        ...
        if -1 != self.selectedLine:
            if self.selectedLine >= self.basePos:
                if self.selectedLine - self.basePos < self.showLineCount:
                    ...
                    grp.RenderBar(int(xRender + 2), int(yRender + (self.selectedLine-self.basePos)*self.stepSize + 4), int(self.width - 3), int(self.stepSize))
...
class ListBox2(ListBox):
    ...
    def _RenderBar(self, basePos, idx):
        ...
        grp.RenderBar(int(x), int(y), int(self.barWidth - 3), int(self.stepSize))
...
class BorderA(Window):
    ...
    def __init__(self, layer="UI"):
        ...
        CornerFileNames = ["d:/ymir work/ui/pattern/border_a/border_a_" + dir + ".tga" for dir in
                           ["Left_Top", "Left_Bottom", "Right_Top", "Right_Bottom"]]
        LineFileNames = ["d:/ymir work/ui/pattern/border_a/border_a_" + dir + ".tga" for dir in
...
class CheckBox(Window):
    ...
    def __init__(self, layer = "UI"):
        ...
        self.btnBox = {
            self.STATE_UNSELECTED : self.__init_MakeButton("d:/ymir work/ui/new/checkbox/checkbox_new_unselected.tga"),
            self.STATE_SELECTED : self.__init_MakeButton("d:/ymir work/ui/new/checkbox/checkbox_new_selected.tga", "d:/ymir work/ui/new/checkbox/checkbox_new_selected.tga"),
        }
