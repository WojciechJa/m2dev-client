...
class ConnectingDialog(ui.ScriptWindow):
    ...
    def OnUpdate(self):
        lastTime = max(0, self.endTime - _now())
        if 0 == lastTime:
            self.Close()
            self.eventTimeOver()
        else:
            self.SetCountDownMessage(self.endTime - _now())
...
class LoginWindow(ui.ScriptWindow):
    ...
    def __LoadScript(self, fileName):
        ...
        try:
            ...
            self.virtualKeyboard = self.GetChild2("VirtualKeyboard")
            if not ENABLE_VIRTUAL_KEYBOARD and self.virtualKeyboard:
    ...
    def __LoadLoginInfo(self, loginInfoFileName):
        ...
        candidates = [loginInfoFileName]
        baseLower = loginInfoFileName.lower()
        if baseLower.endswith(".xml"):
            candidates.append(loginInfoFileName[:-4] + ".py")
            candidates.append(loginInfoFileName[:-4] + ".txt")
        elif baseLower.endswith(".py"):
            candidates.append(loginInfoFileName[:-3] + ".xml")
            candidates.append(loginInfoFileName[:-3] + ".txt")
        else:
            candidates.append("loginInfo.py")
            candidates.append("loginInfo.xml")
        ...
        try:
            exec(compile(raw, usedName, 'exec'), loginInfo)
        except Exception:
            # Fallback: parse simple key=value file (supports quotes)
            try:
        ...
        slot=loginInfo.get("slot", 0)
        isAutoLogin = loginInfo.get("autoLogin", loginInfo.get("auto", 0))
        isAutoSelect=loginInfo.get("autoSelect", 0)
